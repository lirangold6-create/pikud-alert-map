...
function getRegionalProbabilityMultiplier(cityRegion, attackPattern, multiMissileInfo = null, cityName = null) {
  ...
  // These regions often appear together in large attacks, but conversion rates differ.
  // Penalties should be moderate — distance and historical rates handle the fine tuning.
  ...
  if (isMutuallyExclusive) {
    ...
    // Moderate penalty — don't crush predictions (distance curve handles the rest)
    if (dominance >= 0.70) return 0.65;
    if (dominance >= 0.55) return 0.75;
    return 0.85;
  }
